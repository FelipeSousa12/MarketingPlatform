...
require_once '../BancoDeDados/Conexao_Banco_Plataforma.php.inc';
...
class daoGenerico extends ConexaoDB {
   ...
   public function retornarCnpj($num){
     $sql = "SELECT ID_ANUNCIANTE AS ID FROM ANUNCIANTE WHERE CNPJ = '$num'";
     $valor = $this->executaSQL($sql);
     return $valor;
   } 


   public function inserirPontos($sql){
     $result = $this->executaSQL($sql);
     return $result;
   }

   public function listarPontos($sql){
     $result = $this->executaSQL($sql);
     return $result;
   }
